...
/*** Rewritten by Galnor on 01/29/2009 ***/
int Descriptor::ProcessOutput()
{
/***
	int result;
	output += "\r\n";
...
//	if ( STATE( this ) == CON_PLAYING && this->character && !PRF_FLAGGED( this->character, PRF_COMPACT ) )
//		output += "\r\n";

	result = WriteToDescriptor(this, output.c_str());
	if ( this->snoop_by && this->snoop_by->HasPermissionToSnoop() )
	{
		SEND_TO_Q( "% ", this->snoop_by );
		SEND_TO_Q( output.c_str(), this->snoop_by );
	}
	output.clear();
	return result;
***/
...
int WriteToDescriptor( Descriptor *d, const char *txt )
{
	if ( !d )
		return -1;

	if ( d->character && PLR_FLAGGED( d->character, PLR_LOGGER ) )
		d->character->LogOutput( txt );
	d->descriptor->socketWriteInstant(txt);
}
