...
function bootQueditOLC()
{
	...
	mode.display = function(actor) {
		...
		if ( quest && actor.getOLC().loaded == false ) {
			...
			for each ( var check in quest.extras ) {
				actor.getOLC().extras.push(check);
			}
			for each ( var num in quest.open ) {
				actor.getOLC().open.push(num);
			}
			for each ( var tag in quest.tags ) {
				actor.getOLC().tags.push(tag);
			}
			for each ( var item in quest.items ) {
				actor.getOLC().items.push(item);
			}
			for each ( var item in quest.itemReward ) {
				actor.getOLC().itemReward.push(item);
			}
			for each ( var thing in quest.dialogue ) {
				actor.getOLC().dialogue.push(thing);
			}
			for each ( var skill in quest.skillArray ) {
				actor.getOLC().skillArray.push(skill);
			}
			for each ( var task in quest.taskArray ) {
				if ( task[1] == "null" )
					task[1] = null;
				actor.getOLC().taskArray.push(task);
			}
			for each ( var name in quest.priorQuests ) {
				actor.getOLC().priorQuests.push(name);
			}
			for each ( var name in quest.editors ) {
				actor.getOLC().editors.push(name);
			}
		}
	...
	mode.display = function(actor) {
		...
		if ( action == -1 ) {
			...
			var val = actor.getOLC().val;
			if ( val == 0 )
				actor.send(bld+"Enter task string: "+nrm);
			else if ( val == 1 )
