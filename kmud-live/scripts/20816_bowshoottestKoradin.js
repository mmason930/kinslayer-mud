var script20816 = function(self, actor, here, args, extra)
{
...
var shotStageCharMsgs = [ "You nock " + arrow + " and begin to study the area.",
						  ...
						  "\nYou start to draw " + arrow + " back further, further increasing your shot's power.",
...
//	actor.deletePval("timerLock", true);
...
//if (actor.getPval("timerLock"))
//{
//	actor.comm(actor.getPval("timerLock"));
//	actor.deletePval("timerLock", true);
//}
wait 4;
...
/*	if( shotStage > 2 && !vict.isBashed && canBeStunned && vict.isValid ) {
		...
		if( stunRoll < stunSave && vict.dodge / 3.0 > vict.absorb ) {
			...
			act("The impact from " + arrow + " knocks you backwards, but you manage to roll back to your feet.", true, ch, weapon, vict, constants.TO_VICT);
		}
			...
			act("You flinch as " + arrow + " hits you, but you avoid getting knocked over.", true, ch, weapon, vict, constants.TO_VICT);
		}
		else {
			// Bash your victim for a round.
			act("$N is knocked off $S feet and sent sprawling!", true, ch, weapon, vict, constants.TO_CHAR);
			act(capFirstLetter(arrow) + " knocks you off your feet, sending you sprawling!", true, ch, weapon, vict, constants.TO_VICT);
