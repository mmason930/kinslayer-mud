var script19509 = function(self, actor, here, args, extra) {
	...
	if( rand == 1 )
	    here.echo(capFirstLetter(self.name) + " sniffs at something on the ground...");
	else if( rand == 2 )
	    here.echo(capFirstLetter(self.name) + " purrs softly.");
	else if( rand == 3 )
	{
	    target = getRandomPersonInRoom(here);
	    if( target != self && self.canSee(target) )
	    {
	        here.echoaround(target, capFirstLetter(self.name) + " growls at " + target.name + ".");
	        target.send(capFirstLetter(self.name) + " growls at you.");
	    }
	}
};