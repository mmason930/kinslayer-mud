var script23202 = function(self, actor, here, args, extra)
{
...
if( !self.init ) {
    ...
    self.getDistanceTo = function( x, y )
    {
        return Math.sqrt( Math.pow(self.getShipX() - x, 2) + Math.pow(self.getShipY() - y, 2) );
    }
    /*** Move the ship forward one tile, based on its current heading ***/
    self.moveShipForward = function()
    {
		...
		//Notify any ship that sees another one pass close by.
        if( self.getDistanceTo( oS.getShipX(), oS.getShipY() ) <= 1 )
        {//If the other ship is just one tile away, its one room adjacent to us. Let's notify the passengers of this.
            var dir = constants.SOUTH;
			if( self.getShipX() > oS.getShipX() )
                dir = constants.EAST;
            else if( self.getShipX() < oS.getShipX() )
                dir = constants.WEST;
			else if( self.getShipY() > oS.getShipY() )
				dir = constants.NORTH;
            self.sendToOutdoors("Very close to the " + dirToText(revDir(dir)) + ", you see a huge ship plunder by. ");
            oS.sendToOutdoors("Very close to the " + dirToText(dir) + ", you see a huge ship plunder by. ");
        }
		
		//Notify this ship if it is near land.
		var myRoom = getRoom(self.getWaterRoom());
		
		for(var i = 0;i < 6;++i)
		{
		    var neighbor = myRoom.direction(i);
			if( !neighbor )
			    continue;
			if( neighbor.vnum >= 23300 && neighbor.vnum <= 23399 )
			    continue;
			self.sendToShip("There is land to the " + dirToText(i) + " where the ship can dock.");
		}
    }
    /*** Returns the respective water tile, for the current ship's location ***/
