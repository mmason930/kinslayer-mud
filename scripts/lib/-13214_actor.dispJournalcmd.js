...
JSCharacter.prototype.dispJournal = function(cmd) {
	...
	var ownQuests = Quest.getQuestsFor(this);
	for (var i = 0, len = ownQuests.length; i < len; i++) {
		var quest = ownQuests[i];
		var qName = quest.name;
		if (quest.hasCompleted(this)) {
			completedEntries.push(quest);
		}
		else if (quest.readyToComplete(this)) {
			readyEntries.push(quest);
		}
		else if (quest.anyTaskStarted(this)) {
			progressEntries.push(quest);
		}
		else {
			newEntries.push(quest);
		}
		allEntries.push(quest);
	}
	...
	if (strn_cmp(cmd, "CLOSE JOURNAL", 1) == true) {
		...
		if (journal.chapterPage[0] != 0 && journal.chapterPage[1] != 0 && journal.chapterPage[0] < 5) {
			// this.send("Setting [2] to "+arrayChapters[journal.chapterPage[0]][journal.chapterPage[1]].name);
			// The following line stores the current chapter and page in a third index of journal.chapterPage
			journal.chapterPage[2] = arrayChapters[journal.chapterPage[0]][journal.chapterPage[1]].databaseID;
		}
	...
	if (strn_cmp(cmd, "REREAD", 1) == true) {
		...
		if (journal.chapterPage[2] != undefined && journal.chapterPage[2]) {
			for (var i = 1; i < arrayChapters.length; i++) {
				var check = false;
				...
				if (chapter != undefined) {
					for (var j = 0; j < chapter.length; j++) {
						var entry = chapter[j];
						if (entry && entry.databaseID == journal.chapterPage[2]) {
...
JSCharacter.prototype.chapterCompile = function(arrayChapters, intTargetChapter, intTargetPage) {
	...
	if (intTargetChapter == 0) {
		...
		journal.chapterPage[0] = intTargetChapter;//this puts the actor in the TOC Tier
			...
			if (intTargetPage == 0 || intTargetPage == (pageLimit + 1)) {
				...
				if (intTargetChapter < 6) {
					for (var i = 1; i < entriesArray.length; i++) {
						var quest = entriesArray[i];
						var qName = quest.name;
						var mobName = getMobName(quest.giverVnum(this));
