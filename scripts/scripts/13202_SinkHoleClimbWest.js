var script13202 = function(self, actor, here, args, extra) {
	if( extra.direction == constants.WEST ) {
		...
		here.echoaround(actor, capFirstLetter(actor.name) + " begins climbing " + his_her(actor.sex) + " way to the west.");
		actor.startTimer(3);
		...
		{//25% chance of slipping back in
		    if( random(1,100) <= 25 ) {
				...
				here.echoaround(actor, capFirstLetter(actor.name) + " manages to pull " + him_her(actor.sex) + "self to the west.");
