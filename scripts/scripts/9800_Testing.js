var script9800 = function(self, actor, here, args, extra) {
	...
	let weap = actor.eq(constants.WEAR_WIELD);
	let target = actor.getCharRoomVis(args.split(" ")[1]);
	let mindmg = weap.value(constants.VALUE_WEAPON_BACKSTAB_LOW);
	let maxdmg = weap.value(constants.VALUE_WEAPON_BACKSTAB_HIGH);
	let dmg = random(mindmg, maxdmg);
	let landroll = random(1, 100);
	actor.startTimer(3);
	runTimer(actor)

	if (landroll < weap.calue(constants.VALUE_WEAPON_BACKSTAB_RATING) + 1) {
