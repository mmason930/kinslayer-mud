var script385 = function(self, actor, here, args, extra) {
	...
	var task = quest.tasks[1];
	if (task.isInProgress(actor) && actor.checkQuestItem("Murash's order of attack on Caemlyn",1,quest) ) {
		_block;
		waitpulse 7;
		...
		// self.attach(386);
		actor.remQuestItem("Murash's order of attack on Caemlyn", quest)
	}
}